...
const img = (id: number) =>
  `https://images.unsplash.com/photo-${1500000000000 + id}?w=800&h=450&fit=crop`;
...
export function searchArticles(query: string, filters?: { category?: string; source?: string }) {
  ...
  return newsArticles.filter((a) => {
    ...
    const matchesSource = !filters?.source || a.source === filters.source;
